...
public class MaBibliothequeTraitementImageEtendue {
	...
	public static double Similitude(Mat object,String signfile) {
		...
		//			moyenne+=distances[i];
		//		}
		//		double total = moyenne/distances.length;
		//		System.out.println(total);
		//		return total;

		// Pixels blancs
		//		int pixelsBlancs = 0; for (int i=0; i<grayObject.height();i++) { 
		//			for (int j=0; j<grayObject.width();j++) {
		//				double grayPixelObject = grayObject.get(i,j)[0];
		//				double grayPixelSign = graySign.get(i,j)[0];
		//				if((grayPixelObject+grayPixelSign)/2>=220 ) { 
		//					pixelsBlancs++; 
		//				}
		//			} 
		//		}
		...
		// Restriction de l'image à la zone qui nous intéresse
		for (int i=0;i<grayObject.height();i++) {
			for (int j=0; j<grayObject.width(); j++) {
		...
		try {
			...
			//			if (signfile.equals("refdouble.jpg")) {
